...
export interface SqlFragment<T, K extends string | never, C> {
  ...
  column: TableColumn<T, any, any> | undefined
 ...
 *   sql`${table.column} >= ${sql.number('key')}`
 *
 * If you need more than 1 parameter key, pass multiple sql fragments to the method, e.g. whereSql:
 *
 *   whereSql(
 *     sql`${table.column} BETWEEN ${sql.number('low')}`,
 *     sql`AND ${sql.number('high')}`,
 *   )
 */
export interface SqlFragmentBuilder {
  ...
  param<C, K extends string>(
    table: TableColumn<any, any, C>,
    key: K,
  ): SqlFragmentParam<K, C>
