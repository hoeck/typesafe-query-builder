...
- proper insert and update semantics:
  - use `undefined` as the neutral value for insert / updates  to e.g. force using an autogenerated ID (insert) to force not touching sensitive id fields:
  ```
       Table.update(tx, params, {...data, id: undefined, permissions: undefined}) // if data contains an id or a roque permissions key it will be ignored by update
  ```
